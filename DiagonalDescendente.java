 ...
 * Classe utilizada para a verificação dos vários aspetos de uma diagonal descendente (fila diagonal).
...
public class DiagonalDescendente implements Fila{
    ...
    /**
     * Construtor da classe DiagonalDescendente, que guarda nas variáveis de instância da classe os valores recebidos como argumento.
     *
     ...
     */
    public DiagonalDescendente(Tabuleiro tab, int linha, int coluna) {
        this.tabuleiro = tab;
        this.linha = linha;
        this.coluna = coluna;
    }
     ...
     * No primeiro ciclo for é verificada a parte direita da diagonal descendente (parte inferior).
     * No segundo ciclo for é verificada a parte esquerda da diagonal descendente (parte superior).
    ...
    public int comprimento() {
        int contador = -1;
        for (int i = this.linha, j = this.coluna; i < this.tabuleiro.getTamanho() && j <this.tabuleiro.getTamanho(); i++, j++) {
            contador++;
        }
     ...
     * Assim que o limiteLinha ou o limiteColuna sejam 0, o ciclo while não é mais executado.
     *
     * @param pos variável que indica a posição da peça na diagonal
     * @return retorna a peça presente na posição pos da diagonal (incrementando pos ao limiteLinha e ao limiteColuna)
    ...
    public Peca peca(int pos) throws IndexOutOfBoundsException {
        ...
        while (limiteLinha > 0 && limiteColuna > 0){
            limiteColuna--;
            limiteLinha--;
        }
